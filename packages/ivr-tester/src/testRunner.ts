...
import { validateConfig, validateGenesysConfig } from "./configuration/validateConfig";
...
import { GenesysCallServer } from "./testing/GenesysCallServer";
...
function createPluginManager(config: Config | GenesysConfig): PluginManager {
  return new PluginManager([
    new StopTestRunnerWhenTestsComplete(),
    consoleUserInterface(),
    callConnectedTimeout(config),
    mediaStreamPlayerPlugin(config),
...
export class IvrTester implements RunnableTester {
  ...
  public async run(
    ...
    const callServer = new TwilioCallServer(
      ...
      testExecutor(this.config.transcriber , this.config.synthesizer)
    );

    const twilioClient = this.config.twilioClientFactory(
      this.config.twilioAuth
    );
    const caller: Caller<IvrNumber | Buffer| GenesysNumber> = Buffer.isBuffer(testSubject)
...
export class IvrGenesysTester implements RunnableTester {
  private readonly config: GenesysConfig;
  private readonly pluginManager: PluginManager;
  private running = false;

  constructor(configuration: GenesysConfig) {
    const result = validateGenesysConfig(configuration);
    if (result.error) {
      throw result.error;
    }
    if (!result.config) {
  ...
  public async run(
    ...
    const callServer = new GenesysCallServer(
      this.config.dtmfGenerator,
      new IteratingTestAssigner(scenarios),
      testExecutor(this.config.transcriber , this.config.synthesizer),
      this.config.genesysAuth
    );

    const caller: Caller<GenesysNumber | Buffer> = Buffer.isBuffer(testSubject)
      ? new AudioPlaybackCaller()
      : new GenesysCaller(this.config.genesysAuth);

    const testRunnerManager = createTestRunnerManager();
    this.pluginManager.initialise(testRunnerManager.testRunner);

     await callServer.listen(undefined);
     this.pluginManager.serverListening(callServer); //To be changed

    const calls = Promise.all(
      scenarios.map(() =>
        caller
          .call(testSubject) //startSession
          .then((callRequested) =>
            this.pluginManager.callRequested(callRequested, scenarios.length)
          )
          .catch((error) => {
