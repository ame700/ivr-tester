...
export const transcriptRecorderPlugin = (config: Config | GenesysConfig): IvrTesterPlugin => {
...
export class TranscriptRecorder {
  ...
  private saveTimedOutPromptThenClose(event: TimeoutWaitingForMatchEvent) {
    ...
    if (this.config.includeResponse) {
      prompt.push(`${recipientName}: ${event.transcription}`);
      prompt.push(
        "Bot: Ended test as prompt did not match condition within timeout period"
      );
    } else {
  ...
  private saveMatchedPrompts(event: PromptMatchedEvent) {
    ...
    if (this.config.includeResponse) {
      prompt.push(`${recipientName}: ${event.transcription}`);
      prompt.push(`Bot: ${event.promptDefinition.then.describe()}`);
    } else {
      prompt.push(`${event.transcription}`);
    }
